<template>
  <div class="p-8">
    ...
    <div class="space-y-4">
      ...
      <!-- Chat Paper Section -->
      <div class="space-y-2">
        <h2 class="text-xl font-semibold">Chat Paper</h2>
        <div class="flex space-x-2 items-center my-2">
          <input
            v-model="chatPaperId"
            type="text"
          ...
          <input
            ...
            class="input-field flex-grow"
          />
          <button @click="testChatPaper" class="btn">
            Send Message
          </button>
        </div>
      </div>
...
async function testChatPaper() {
  ...
  const result = await ChatPaper(chatPaperId.value, messages);
  logResult("Chat Paper", result);
}
</script>
