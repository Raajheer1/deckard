<template>
  <div class="flex items-center justify-center mt-[100px]">
    ...
    >
      <!-- Title -->
      <h1 class="text-4xl font-semibold flex mx-auto mb-4">
        Register for
        <span
          class="flex mx-auto border-b-4 border-b-primary ml-2"
        >
          <img
            src="@/assets/wallace_lighter.png"
            alt="Logo"
        ...
        <div class="flex flex-col gap-2">
          <FloatLabel variant="on">
            <InputText
              ...
              class="w-full bg-surface-0 text-black placeholder:text-gray-500"
            />
            <label for="Enter a preference"
              >Enter a Preference</label
            >
          </FloatLabel>
        </div>
      </form>

      <div class="flex flex-wrap justify-center gap-4 w-1/2">
        <Chip
          v-for="(opt, idx) in preferenceOptions"
          :key="idx"
          :label="opt"
          @click="clickPreference(opt)"
          class="cursor-pointer hover:-translate-y-1 transition duration-200"
          :class="
            selectedPrefs.includes(opt)
